...
public interface OrderMapper {
    ...
    @Select("SELECT" +
            ...
            "FROM" +
            "ycb_mcs_tradelog t" +
            "LEFT JOIN ycb_mcs_shop_station ssout ON t.borrow_shop_station_id = ssout.id" +
            "LEFT JOIN ycb_mcs_fee_strategy f ON ssout.fee_settings = f.id" +
            "LEFT JOIN ycb_mcs_shop_station ssin ON t.return_shop_station_id = ssin.id" +
            "WHERE" +
            "t.customer = 10" +
            "AND t. STATUS <> 0" +
            "ORDER BY" +

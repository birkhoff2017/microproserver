...
public interface OrderMapper {
    ...
    @Results(id = "station", value = {
            ...
            @Result(property = "feeStrategyEntity", column = "fee_settings", one = @One(select = "com.ycb.wpc.provider.mapper.FeeStrategyMapper.findFeeStrategy"))
    })
    ...
    //根据小程序openid查询某条订单的具体信息
    @Select("SELECT borrow_station_id,borrow_time,borrow_station_name,return_station_name,return_time,paid,orderid FROM ycb_mcs_tradelog WHERE platform = 3 AND customer = (SELECT id FROM ycb_mcs_user WHERE openid=#{srOpenid})")
    @Results(value = {
    ...
    TradeLog findOrderBySrOpenid(@Param("srOpenid") String srOpenid);
}

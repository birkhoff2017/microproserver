...
import com.ycb.wpc.provider.service.MessageService;
import com.ycb.wpc.provider.utils.*;
import com.ycb.wpc.provider.vo.Order;
...
public class RefundController {
    ...
    @Value("${refundSendTemplateId}")
    private String refundSendTemplateId;

    // 获取用户信息
    @RequestMapping(value = "/getUserMessage", method = RequestMethod.POST)
    @ResponseBody
    public String queryUserMessage(@RequestParam("openid") String openid) {
        Map<String, Object> bacMap = new HashMap<>();
        try {
            User user = this.userMapper.findUserMessageyOpenid(openid);
    ...
    public String query(@RequestParam("openid") String openid) {
        Map<String, Object> bacMap = new HashMap<>();
        try {
            //String srOpenid = this.userMapper.findSrOpenidByOpenid(); //通过
    ...
    @ResponseBody
    public String wechatRefund(@RequestParam("openid") String openid) throws UnsupportedEncodingException {
        ...
        if (null == orderList || 0 == orderList.size()) {
            ...
            for (int i = 0; i < orderList.size(); i++) {
                ...
                refundMoney = refundMoney.add(orderList.get(i).getPaid()).subtract(orderList.get(i).getUsefee());//退款金额
